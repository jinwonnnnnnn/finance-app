...
async function callGroq(messages) {
  const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({ model: 'llama-3.3-70b-versatile', messages, max_tokens: 4096, temperature: 0.1 }),
  });
...
async function main() {
  ...
  try {
    await updateComment(`## 🤖 AI Issue Engineer 작동 중\n\n이슈를 분석하고 있습니다...`);
  ...
  "files": [
    {
      ...
      "old_snippet": "EXACT text to find (must be unique in file, at least 40 chars)",
...
Rules:
- old_snippet must EXACTLY match existing file content character-by-character
...
## 현재 파일 내용
${fileSection || '(이슈 내용을 바탕으로 수정할 파일을 추론하세요)'}
...
위 이슈를 수정하세요. JSON만 반환하세요.`;
    ...
    if (!parsed.files || parsed.files.length === 0) {
      await updateComment(`## ⚠️ AI Issue Engineer\n\nAI가 수정할 파일을 찾지 못했습니다.\n이슈에 파일 경로와 오류 내용을 더 구체적으로 작성해주세요.`);
      return;
    }

    execSync(`git config user.email "ai-bot@github-actions"`);
    execSync(`git config user.name "AI Issue Engineer"`);
    execSync(`git checkout -b ${branchName}`);

    const applied = [];
    const failed = [];

    for (const { path, old_snippet, new_snippet } of parsed.files) {
      try {
        if (!existsSync(path)) {
          writeFileSync(path, new_snippet, 'utf-8');
          execSync(`git add "${path}"`);
          applied.push(`✅ \`${path}\` (새 파일)`);
        } else {
          const content = readFileSync(path, 'utf-8');
          if (!content.includes(old_snippet)) {
            failed.push(`❌ \`${path}\` — old_snippet이 파일에 없음`);
            continue;
          }
          writeFileSync(path, content.replace(old_snippet, new_snippet), 'utf-8');
          execSync(`git add "${path}"`);
          applied.push(`✅ \`${path}\``);
        }
      } catch (err) {
    ...
    if (applied.length === 0) {
      await updateComment(`## ❌ AI Issue Engineer 실패\n\n파일 수정에 실패했습니다:\n\n${failed.join('\n')}`);
      return;
    }
    ...
    await updateComment([
      `## ❌ AI Issue Engineer 실패`,
      ``,
      '```',
      err.message,
      '```',
      ``,
      `> 이슈에 \`@ai 다시 시도\` 댓글을 달면 재시도합니다.`,
    ].join('\n'));
    process.exit(1);
  }
}
