...
// Yahoo Finance v8 차트 엔드포인트 (range 기반, crumb 불필요)
...
export default async function handler(req: any, res: any) {
  ...
  if (type === 'search') {
    ...
    url = `${YF_SEARCH}?q=${encodeURIComponent(q)}&quotesCount=${quotesCount}&newsCount=0&enableFuzzyQuery=false`;
  } else {
    if (!symbol) return res.status(400).json({ error: 'symbol required' });
    url = `${YF_CHART}/${symbol}?interval=${interval}&range=${range}&includePrePost=false`;
  }

  try {
    const yfRes = await fetch(url, { headers: YF_HEADERS });
    const data = await yfRes.json();
    // YF 4xx/5xx는 그대로 전달 — 백엔드가 status로 분기 가능
    res.setHeader('Cache-Control', 's-maxage=20, stale-while-revalidate=40');
    res.status(yfRes.status).json(data);
  } catch (e: any) {
